...
class WizardTests extends GrailsUnitTestCase {
    ...
    void testFirst2StepsOk() {
        ...
        assertFalse(w.finished)
        assertEquals(Response.build(SetEndDate.INSTANCE.msgCode).message, conversation.responses[0].message)
        assertEquals('proyecto', conversation.context.project.name)
    ...
    void testFirst3StepsErrorAndResume() {
        ...
        assertEquals(98, conversation.context.project.startDate.year)
        assertNull(conversation.context.project.endDate)
        assertEquals(Response.build(SetEndDate.INSTANCE.errorMsgCode).message, conversation.responses[0].message)
        assertEquals(Response.build(SetEndDate.INSTANCE.msgCode).message, conversation.responses[1].message)
